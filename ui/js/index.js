...
app.controller('WorkflowsController', ['$scope', 'menu', 'scaMessage', 'toaster', 'jwtHelper', 'workflows', '$location',
function($scope, menu, scaMessage, toaster, jwtHelper, workflows, $location) {
    ...
    $scope.detail = function(wid) {
        $location.path("/workflow/"+wid);
    }
    $scope.openinst = function(wid, instid) {
        //window.location = $scope.appconf.api+"/workflow/html/"+instid;
        window.location = $scope.workflows[wid].url+"#/"+instid+"/start";
    }
    $scope.create = function(wid) {
        workflows.createInst(wid).then(function(res) {
            scaMessage.success("Created a new workflow instance"); //TODO unnecessary?
            $scope.openinst(wid, res.data._id);
        }, function(res) {
            if(res.data && res.data.message) toaster.error(res.data.message);
            else toaster.error(res.statusText);
        });
...
app.controller('WorkflowController', ['$scope', 'menu', 'serverconf', 'scaMessage', 'toaster', 'jwtHelper', 'workflows', '$location', '$routeParams',
function($scope, menu, serverconf, scaMessage, toaster, jwtHelper, workflows, $location, $routeParams) {
    scaMessage.show(toaster);
    workflows.get().then(function(workflows) {
...
app.controller('InstsController', ['$scope', 'menu', 'serverconf', 'scaMessage', 'toaster', 'jwtHelper', 'workflows', '$location',
function($scope, menu, serverconf, scaMessage, toaster, jwtHelper, workflows, $location) {
    scaMessage.show(toaster);
